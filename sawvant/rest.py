...
from sawvant.exceptions import ApiException, ApiValueError
...
class RESTClientObject:
    ...
    def request(
        ...
        # Set SDK User-Agent
        headers.setdefault('User-Agent', 'sawvant-python-sdk/0.1.0')
            ...
            try:
                if method in ['POST', 'PUT', 'PATCH', 'OPTIONS', 'DELETE']:
                    ...
                    if not content_type or re.search('json', content_type, re.IGNORECASE):
                        r = self.pool_manager.request(
                            method, url,
                            body=request_body if 'request_body' in dir() else None,
                            timeout=timeout, headers=headers, preload_content=False
                        )
                    else:
                        r = self.pool_manager.request(
                    ...
                    r = self.pool_manager.request(
                        method, url, fields={}, timeout=timeout,
                        headers=headers, preload_content=False
                    )
            except urllib3.exceptions.SSLError as e:
                msg = "\n".join([type(e).__name__, str(e)])
